...
[^ArrayList]: 기본값으로 0, 하나라도 원소를 추가한다면 10으로 시작
[^HashMap]: 기본값으로 initial capacity 16, load factor 0.75
...
원인을 추적해보니, DB에서 CLOB/BLOB을 읽는 Reader 구현이 `char[]`를 매번 두 배씩 늘려가며 새로 할당하는 방식으로 작성되어 있었습니다.
...
public String readClob(Reader reader) throws IOException {
    ...
    while ((n = reader.read(buf, totalRead, buf.length - totalRead)) != -1) {
        ...
        if (totalRead == buf.length) {
            char[] bigger = new char[buf.length * 2]; // 매번 새 배열 할당
...
실제 운영 환경에서 오라클 SQL 쿼리 하나로 CLOB를 사용해 A4용지 100페이지 가량의 HTML을 생성하는 경우가 종종 있는데, 앞선 로직에서 이 배열이 수십~수백 회 재할당되고, 최종 결과물 역시 수 MB짜리 `char[]`가 됩니다.
