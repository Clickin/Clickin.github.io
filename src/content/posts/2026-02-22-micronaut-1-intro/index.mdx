---
title: "Compile-time DI의 시대를 열다 — Micronaut 소개와 역사적 맥락"
...
> 시리즈의 다음 포스팅들은 본격적으로 Micronaut를 다루지만, 이 포스팅은 DI 의 개념이 태동한 시기부터 최근까지의 연혁을 서술하는데 집중합니다.   
> Micronaut에 관심이 없으시더라도, 만약 DI의 역사가 궁금하시다면 한번쯤 읽어주세요.
...
### Compile-time DI란 무엇인가

Micronaut의 핵심 혁신은 DI 처리를 **컴파일 타임으로 옮긴 것**입니다. Spring이 런타임에 reflection으로 하던 작업들, 즉 어노테이션 분석, 의존성 해석, 프록시 생성을 모두 **javac의 annotation processing 단계**에서 처리합니다.

이 방식은 AOT(Ahead-Of-Time) 사상과 같은 방향입니다. Java 생태계에서 AOT의 대표 사례인 GraalVM Native Image는 JVM이 런타임에 JIT 컴파일로 생성하는 코드를 미리 컴파일해서 네이티브 바이너리로 만들어두는 것입니다. Micronaut의 compile-time DI 역시 동일한 원칙, 즉 "런타임 이전에 미리 처리한다"는 철학에서 출발하지만, 그 구현은 GraalVM이 아닌 **Java annotation processor(APT)**로 이루어집니다.
...
Micronaut은 클라우드, 컨테이너, 서버리스라는 새로운 실행 환경이 요구하는 특성, 빠른 시작, 낮은 메모리, GraalVM 호환성을 compile-time DI로 달성하려 했습니다.

1편의 핵심을 한 문장으로 정리하면: **Reflection 기반 DI는 2003년의 기술 환경에서 합리적인 선택이었고, Micronaut은 그 선택이 맞지 않는 실행 환경이 생겼을 때 등장했습니다.**

"compile-time DI가 실제로 어떤 코드를 생성하는가"는 3편에서 자세히 다룹니다. 2편에서는 먼저 Micronaut으로 첫 프로젝트를 만들어봅니다.
